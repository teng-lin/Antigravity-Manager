                ...
                {account.quota?.is_forbidden ? (
                    ...
                    <>
                        <div className="grid grid-cols-2 gap-2">
                            {models.map((model) => (
                                <div key={model.name} className="relative h-[26px] flex items-center px-1.5 rounded-lg overflow-hidden border border-gray-100/50 dark:border-white/5 bg-gray-50/30 dark:bg-white/5 group/quota">
                                    <div
                                    ...
                                    <div className="relative z-10 w-full flex items-center justify-between text-[10px] font-mono leading-none">
                                        <span className="text-gray-500 dark:text-gray-400 font-bold truncate pr-1" title={model.name}>
                                        ...
                                        <div className="flex items-center gap-1 shrink-0">
                                            {model.reset_time && (
                                                <span className="text-gray-400 dark:text-gray-500 scale-90 opacity-40 group-hover/quota:opacity-100 transition-opacity whitespace-nowrap">
                                                    R:{formatTimeRemaining(model.reset_time)}
                                                </span>
                                            )}
                                            <span className={`font-bold ${getQuotaColor(model.percentage) === 'success' ? 'text-emerald-600 dark:text-emerald-400' : getQuotaColor(model.percentage) === 'warning' ? 'text-amber-600 dark:text-amber-400' : 'text-rose-600 dark:text-rose-400'}`}>
                                                {(model.percentage || 0)}%
                                            </span>
                                        </div>
                                    </div>
